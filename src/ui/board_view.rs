...
pub fn show(
    ...
    if let Some(layout) = Layout::fit(response.rect, size) {
        ...
        // 区域模式下排除入口整体停用（含侧栏候选行「排除」按钮）：引擎
        // 实测 allowMoves 与 avoidMoves 互斥（同时给出会被显式拒绝），让
        // 二者"组合生效"并无协议通路；照常收进列表只会静默失效误导用户。
        // 停用的可见性由侧栏承担：候选行「排除」置灰并说明原因，
        // 「限定选点」卡显示 WARN 说明行（棋盘右键在区域模式下本就是
        // 「清区域」语义，不混入排除提示）。
        if region_mode {
